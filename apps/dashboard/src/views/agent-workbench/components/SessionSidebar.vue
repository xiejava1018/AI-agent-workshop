<script setup lang="ts">
  ...
  async function handleNewSession() {
    // 从 userStore 读真实 userId,传给 create。如果不传,后端 listSessions
    // 走 team-auth 鉴权时会把 userId='default' 的 session 过滤掉,导致
    // 新建的会话进不了侧栏。
...
<template>
  <div class="wb-session-list">
        ...
        <template v-else>
          <div class="wb-session-row" @click="handleSelect(s)">
            <div class="wb-session-title">
              <span v-if="s.available === false" class="wb-session-badge" title="该会话文件不可用"
                >⚠</span
              >
    ...
    <div class="wb-sidebar-section">
      ...
      <div
        v-if="pinnedSessions.length === 0 && unpinnedSessions.length === 0"
...
<style scoped>
  ...
  .wb-session-action-btn {
    ...
    padding: 0;
    transition:
      background-color var(--wb-transition-fast) var(--wb-ease-out),

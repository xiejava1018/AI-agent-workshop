// app/api/v1/users/[id]/roles/route.test.ts
// M4 RBAC 平台中台 — 设置用户的全局角色 集成测试。
...
const TEST_USER_PREFIX = "test-v1-userroles-";
...
function makePutReq(userId: string, opts: { callerId?: string; body?: unknown }): NextRequest {
  const url = `http://localhost:30141/api/v1/users/${userId}/roles`;
...
describe("PUT /api/v1/users/[id]/roles", () => {
  it("returns 401 without x-user-id", async () => {
    ...
    const res = await PUT(
      makePutReq(targetId, { body: { roleCodes: ["member"] } }),
  ...
  it("returns 403 for non-platform-admin", async () => {
    ...
    const res = await PUT(
      makePutReq(targetId, { callerId, body: { roleCodes: ["member"] } }),
      { params: Promise.resolve({ id: targetId }) }
    );
  ...
  it("returns 404 when target user not found", async () => {
    ...
    const res = await PUT(
      makePutReq("nonexistent-id", { callerId, body: { roleCodes: ["member"] } }),
      { params: Promise.resolve({ id: "nonexistent-id" }) }
    );
    expect(res.status).toBe(404);
  });

  it("replaces user's roles atomically", async () => {
    const callerId = await makePlatformAdmin();
    const targetId = await makeTargetUser();
    // 初始绑 platform_admin
    const plat = await prisma.sysRole.findUniqueOrThrow({
      where: { code: "platform_admin" },
    });
    await prisma.userRole.create({
      data: { userId: targetId, roleId: plat.id },
    });
    // 替换为 member
    const res = await PUT(
      makePutReq(targetId, { callerId, body: { roleCodes: ["member"] } }),
    ...
    expect(body.data.roleCodes).toEqual(["member"]);
    // 验证 DB
    const target = await prisma.user.findUniqueOrThrow({
      where: { id: targetId },
      include: { userRoles: { include: { role: { select: { code: true } } } } },
    });
    const codes = target.userRoles.map((ur) => ur.role.code);
    expect(codes).toEqual(["member"]);
  });

  it("clears all roles with empty roleCodes", async () => {
    const callerId = await makePlatformAdmin();
    const targetId = await makeTargetUser();
    const member = await prisma.sysRole.findUniqueOrThrow({
      where: { code: "member" },
    });
    await prisma.userRole.create({
      data: { userId: targetId, roleId: member.id },
    });
    const res = await PUT(
      makePutReq(targetId, { callerId, body: { roleCodes: [] } }),
      { params: Promise.resolve({ id: targetId }) }
    );
    expect(res.status).toBe(200);
    const count = await prisma.userRole.count({ where: { userId: targetId } });
    expect(count).toBe(0);
  });
  ...
  it("returns 400 when roleCode unknown", async () => {
    ...
    const targetId = await makeTargetUser();
    const res = await PUT(
      makePutReq(targetId, { callerId, body: { roleCodes: ["nope"] } }),
      { params: Promise.resolve({ id: targetId }) }
    );

...
- [ ] B0.4 `pnpm --filter @ai-agent-workshop/web test`（后端 47+ 测试）全过
- [ ] B0.5 `pnpm --filter @ai-agent-workshop/dashboard test`（现有 user.test.ts）全过
...
- [ ] T10-A.1 `grep -rn "getMenuByRole\|isPlatformAdmin\|isTeamAdmin" apps/dashboard/src/` 再次确认无外部调用
- [ ] T10-A.2 删除三个函数；若文件其余内容（`Role` 类型、`MenuItem` 接口）也无引用则整文件删除，否则保留类型定义
- [ ] T10-A.3 `pnpm --filter @ai-agent-workshop/dashboard build` 通过（验证无断链）
- [ ] T10-A.4 `pnpm --filter @ai-agent-workshop/dashboard test` 通过
...
- [ ] T10-B.1 删除 `apps/dashboard/src/views/system/dict/`
- [ ] T10-B.2 删除 `apps/dashboard/src/store/modules/dict.ts`
- [ ] T10-B.3 移除 `router/routes/asyncRoutes.ts` 中 Dict 路由节点（含其子项）
- [ ] T10-B.4 移除 `router/routesAlias.ts` 的 `Dict` 枚举
- [ ] T10-B.5 `grep -rn "dict\|Dict" apps/dashboard/src/` 确认无残留引用（排除业务无关命中）
- [ ] T10-B.6 build + test 通过
...
- [ ] T10-C.1 移除模板：`<ElFormItem label="部门">` 整块（line 91-100）
- [ ] T10-C.2 移除 `formData.department_id`、`departmentList` ref、`loadDepartmentList`
- [ ] T10-C.3 移除 `getDepartmentList` import（line 149）与 `Promise.all` 中的 `loadDepartmentList()`（line 593）
- [ ] T10-C.4 移除表格列 `department_name`（line 351）与列配置 `department_id`（line 319）
- [ ] T10-C.5 移除 `user-dialog.vue`（若存在）中 department 相关
- [ ] T10-C.6 build 通过；手动核对 user 页面表单无"部门"项
...
- [ ] T10-D.1 删除 `apps/dashboard/src/views/system/department/`
- [ ] T10-D.2 移除 `router/routes/asyncRoutes.ts` 中 Department 路由节点
- [ ] T10-D.3 移除 `router/routesAlias.ts` 的 `Department` 枚举
- [ ] T10-D.4 移除 `api/system/api.ts` 中 department 相关函数（`getDepartmentList` 等，约 line 124-139）
- [ ] T10-D.5 `grep -rn "department\|Department" apps/dashboard/src/` 确认无残留
- [ ] T10-D.6 build + test 通过
...
- [ ] T8-TEST.1 写失败测试 `apps/dashboard/src/directives/core/__tests__/auth.test.ts`：
  - 有匹配 authMark → 元素保留
  - 无匹配 authMark → 元素从 DOM 移除
  - authList 为空 → 元素移除
  - mounted + updated 生命周期
- [ ] T8-TEST.2 跑测试确认 RED
- [ ] T8-TEST.3 若实现已正确 → GREEN（预期直接过，测试是固化契约）；若有 bug → 修实现
- [ ] T8-TEST.4 覆盖率：该文件 ≥ 80%
...
- [ ] T8-ROUTE.1 评估可测性：若 `RouteRegistry` 可独立实例化 → 写 `router/core/__tests__/routeRegistry.test.ts`（addRoute/removeRoute/去重）；若耦合 router 单例过深 → 改测 `RoutePermissionValidator.validatePath`（纯函数，可测）
- [ ] T8-ROUTE.2 写失败测试 → RED → 实现/修正 → GREEN
- [ ] T8-ROUTE.3 覆盖率达标
...
- [ ] V1 `grep -rn "getMenuByRole\|isPlatformAdmin\|isTeamAdmin" apps/dashboard/src/` → 空
- [ ] V2 `grep -rn "department\|Department" apps/dashboard/src/` → 空
- [ ] V3 `grep -rn "system/dict\|'Dict'" apps/dashboard/src/` → 空
- [ ] V4 后端 `/api/v1/*` 47+ 集成测试全过
- [ ] V5 前端 vitest 全过（user + auth 指令 + 路由）
- [ ] V6 dashboard build 通过
- [ ] V7 web build 通过
- [ ] V8 `api/system/api.ts` 仍存在且被 user/role/menu 页面使用（非死代码）
- [ ] V9 三角色 RBAC 流（platform_admin / team_owner / member）后端集成测试覆盖（已在 T5.13）
- [ ] V10 `/api/admin/*` 鉴权：team OWNER 无 `platform:access` → 403（已在 T7 测试）
- [ ] V11 删除项 release notes 记录（department/dict/getMenuByRole 移除，属破坏性前端变更）

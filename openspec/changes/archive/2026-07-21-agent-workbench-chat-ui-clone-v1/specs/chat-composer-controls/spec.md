...
> 路径 A(Agent 工作台 Vue 端)Composer 底部状态条控件。
> 覆盖 B4(model 选择下拉)+ B5(thinking level 下拉)+ B6(tool preset 切换)。
> 依赖:useAgentSession 扩展 modelList / thinkingLevel / availableThinkingLevels / toolPreset / 持久化方法,api/agent.ts 包装 set_model / set_thinking_level / set_tools 命令。

## Purpose

让 `ChatInput.vue` 底部状态条与 React 端对齐:左侧显示当前 model,中部显示 thinking level,右侧显示 tool preset。每个控件:
- 是**显示 + 切换**两用(点击切换)
- 状态变更后调对应后端命令持久化
- 状态变更后乐观更新 UI(无需等响应)

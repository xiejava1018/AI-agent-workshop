...
- [x] 2.1 创建 `app/[locale]/layout.tsx`：根 layout for locale（next-intl `NextIntlClientProvider` + 注入 `messages`）
...
- [x] 2.5 修改 `middleware.ts` matcher：加 `/{locale}/(login|change-password|dashboard)` 例外（不挡 UI 页面）
...
- [x] 4.2 修改 `app/api/agent/new/route.ts`：删 `cwd` 从 body 读取；改用 `user.lastProjectId` → `project.rootPath`（含 membership check）
- [x] 4.3 修改 `app/api/sessions/route.ts`：实现 3 路并集过滤（self / team admin / share placeholder）

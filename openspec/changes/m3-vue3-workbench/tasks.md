...
- [ ] T3.1 实现 `DelegateAgentTool`（`defineTool`）：执行体内部创建子 `createAgentSessionFromServices` 并 `prompt()`，等待结果返回
- [ ] T3.2 Supervisor 发现子 Agent：注入时按 `teamId`（个人按 `ownerUserId`）查出「可用数字员工目录」作为候选上下文
- [ ] T3.3 三种委派模式：同步 / 并行（≤8）/ 异步（task_id + taskOutput 回填）
- [ ] T3.4 委派护栏：深度 ≤ 3、子 Agent 结果截断 4000 字符、子 Agent 禁用清单（delegate*/remember*/setGoal*/create_employee 等）
...
- [ ] T4.1 `/api/digital-employees`：数字员工 CRUD + 绑定技能/MCP（避开现有 `/api/agent/*` 会话端点）
...
- [ ] T4.5 `/api/admin/mcp`：MCP Server CRUD + 按团队/Agent 绑定

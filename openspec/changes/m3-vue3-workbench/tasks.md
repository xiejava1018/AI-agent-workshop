...
- [x] T4.3 `/api/admin/teams`：团队混合生命周期（创建/成员管理/配额/邀请链接）
...
- [x] T6.1 单一入口 + RBAC 菜单渲染：工作区/我的资源（所有用户）、团队（OWNER）、平台管理（管理员）
...
- [x] T6.9 团队管理屏（OWNER）：团队列表 + 成员管理 + 邀请链接：团队列表 + 创建团队 + 成员管理（角色/移除）+ 配额设置 + 邀请链接
- [x] T6.10 平台管理屏（管理员）：用户/模型/MCP/技能/审计/监控：用户管理 + 模型配置 + MCP 精选库 + 技能精选库 + 审计日志 + 监控大盘（ECharts）
- [x] T6.11 状态管理：Pinia stores（auth / user / team / agent / skill / mcp / session）（auth / user / team / agent / skill / mcp / session）
- [x] T6.12 路由守卫：未登录跳登录页；按角色过滤菜单；API 401 自动刷新 token：未登录跳登录页；按角色过滤菜单；API 401 自动刷新 token
